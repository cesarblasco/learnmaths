...
public class Openchest2 : MonoBehaviour {
    ...
    public GameObject number1Obj, number2Obj, minusObj, equalsObj, minusNumber1, minusNumber2, 
        gate, coin;
    ...
    void Start()
    {
        ...
        inputFieldObj = GameObject.Find("InputField2");
        ...
        if (coll.gameObject.tag == "Player")
        {
            if (anim.GetBool("opened") == false)
            {
                ...
                if (x < 0)
                {
                    ...
                    minusNumber1.transform.position = new Vector3(-6, 0, 0);
                    xIsNegative = true;
                    x *= -1;
                }
                if (y < 0)
                {
                    minusNumber2 =
                        Instantiate(Resources.Load("Prefabs/Minus")) as GameObject;
                    minusNumber2.transform.position = new Vector3(0, 0, 0);
                    yIsNegative = true;
                    y *= -1;
                }
                number1Obj = Instantiate(Resources.Load("Prefabs/Number" + x), new Vector3(-4, 0, 0), Quaternion.identity) as GameObject;
                
                minusObj =
                GameObject.Instantiate(Resources.Load("Prefabs/Minus")) as GameObject;
                minusObj.transform.position = new Vector3(-2, 0, 0);


                number2Obj = Instantiate(Resources.Load("Prefabs/Number" + y), new Vector3(0, 0, 0), Quaternion.identity) as GameObject;
    ...
    private void SubmitInput(string arg0)
    {
        ...
        {
            GameObject go = GameObject.Find("chests_1");
            ProcessInput2 scriptPI = (ProcessInput2)go.GetComponent(typeof(ProcessInput2));
            ...
            else
            {
                ...
                {                   
                    ...
                    coin.transform.position = new Vector3(Random.Range(-7, 7), Random.Range(-4, 0), 0);
                }
            }
        }
    }   
