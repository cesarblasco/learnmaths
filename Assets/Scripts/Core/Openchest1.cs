...
public class Openchest1 : MonoBehaviour {
        ...
        if (coll.gameObject.tag == "Player")
        {
            if (anim.GetBool("opened") == false)
            {
                ...
                if (x < 0)
                {
                    ...
                    minusNumber1.transform.position = new Vector3(-3, 0, 0);
                    xIsNegative = true;
                    x *= -1;
                }
                if (y < 0)
                {
                    minusNumber2 =
                        Instantiate(Resources.Load("Prefabs/Minus")) as GameObject;
                    minusNumber2.transform.position = new Vector3(1, 0, 0);
                    yIsNegative = true;
                    y *= -1;
                }
                number1Obj = Instantiate(Resources.Load("Prefabs/Number" + x), new Vector3(-3, 0, 0), Quaternion.identity) as GameObject;
                minusObj =
                GameObject.Instantiate(Resources.Load("Prefabs/Minus")) as GameObject;
                minusObj.transform.position = new Vector3(-1, 0, 0);
                number2Obj =
                              Instantiate(Resources.Load("Prefabs/Number" + y), new Vector3(1, 0, 0), Quaternion.identity) as GameObject;  
                equalsObj =
                Instantiate(Resources.Load("Prefabs/equals")) as GameObject;
                equalsObj.transform.position = new Vector3(3, 0, 0);
                if (xIsNegative)
                {
                    x *= -1;
                }
    ...
    private void SubmitInput(string arg0)
    {
        ...
        {
            ProcessInput scriptPI = (ProcessInput)gameObject.GetComponent(typeof(ProcessInput));
